...
const GetCsKind: React.FC<GetCsKindProps> = ({ onValueChange, csKind }) => {
  ...
  const fetchOptions = async () => {
    try {
      axios
        ...
        .then((res) => {
          const fetchedData = res.data.map((cskindsetup: { id: number; title: string; calView: boolean }) => [
            cskindsetup.id,
            cskindsetup.title,
            cskindsetup.calView,
          ]);
  ...
  return (
    <FormControl fullWidth>
      ...
      <Select labelId="selectBox-label" id="selectBox" value={selectedOption} label="CS유형" onChange={handleSelectChange}>
        {options.map((option) => (
